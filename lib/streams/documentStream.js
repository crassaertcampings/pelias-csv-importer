...
function getBBox(record) {
    const upper_right_lat = getCaseInsensitive('bbox_upper_right_lat', record);
    const upper_right_lon = getCaseInsensitive('bbox_upper_right_lon', record);
    const lower_left_lat = getCaseInsensitive('bbox_lower_left_lat', record);
    const lower_left_lon = getCaseInsensitive('bbox_lower_left_lon', record);

    if (!(upper_right_lat && upper_right_lon && lower_left_lat && lower_left_lon)) {
        return
    }
...
function processRecord(record, next_uid, stats) {
    ...
    try {
        ...
        if (bbox) {
            pelias_document.setBoundingBox(bbox);
        }
